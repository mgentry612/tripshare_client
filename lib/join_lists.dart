...
class JoinListsPage extends StatefulWidget {
  const JoinListsPage({Key? key}) : super(key: key);

  @override
  _JoinListsPageState createState() => _JoinListsPageState();
}

class _JoinListsPageState extends State<JoinListsPage> {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ListDetailsPage(listId: id, joinable: true)),
        );
      },
...
Future<List<dynamic>?> refreshLists() async {
  ...
  final httpResponse = await http
      .get(Uri.parse('http://127.0.0.1:3000/list/joinable/only'),
